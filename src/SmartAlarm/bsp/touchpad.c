...
#define LYRAT_BOARD_TOUCH_PAD_PLAY          (KEY_CODE_WECHAT)
#define LYRAT_BOARD_TOUCH_PAD_SET           (KEY_CODE_PLAY_NEXT)
#define LYRAT_BOARD_TOUCH_PAD_VOLUME_UP     (KEY_CODE_VOL_UP)
#define LYRAT_BOARD_TOUCH_PAD_VOLUME_DOWN   (KEY_CODE_VOL_DOWN)
    ...
    for (int i = 0; i < TOUCH_PAD_MAX; i++) {
        if (touchpad_group[i] != NULL && touchpad_group[i]->timer == xTimer) {
            ...
            if (value < touchpad_dev->threshold) {
                ...
                switch(touchpad_dev->touch_pad_num){
                    case TOUCH_PAD_NUM4:
                        msg.code = LYRAT_BOARD_TOUCH_PAD_VOLUME_DOWN;
                        break;
                    case TOUCH_PAD_NUM7:
                        msg.code = LYRAT_BOARD_TOUCH_PAD_VOLUME_UP;
                        break;
                    case TOUCH_PAD_NUM8:
                        msg.code = LYRAT_BOARD_TOUCH_PAD_SET;
                        break;
                    case TOUCH_PAD_NUM9:
                        msg.code = LYRAT_BOARD_TOUCH_PAD_PLAY;
                        break;
                    default:
                        msg.code = KEY_CODE_UNDEFINE;
                        LOG_ERR("touch pad (%d) not support, ignore it\n", touchpad_dev->touch_pad_num);
                        break;
                }
...
touchpad_handle_t touchpad_create(touch_pad_t touch_pad_num, uint32_t threshold, uint32_t filter_value, touchpad_trigger_t trigger, uint32_t long_press_sec)
{
