...
static DCL_ERROR_CODE_t dcl_asr_audio_write_packet(DCL_ASR_HANDLE_t *handler)
{
	...
	do
	{
		...
		if (send_len != http_buffer->http_request_len)
		{
			...
			DEBUG_LOGI(TAG_LOG, "dcl_asr_audio_write_packet index[%d], len[%d], send_len[%d], cost[%llums], sock_err[%d]", 
				http_buffer->index, 
	...
	const uint32_t pcm_len)
{
	...
	if (http_buffer->index > 0) 
	{
		...
		do
		{	
			ret = sock_readn_with_timeout(http_buffer->sock, &drop, sizeof(drop),0);
			if (ret > 0)
			{
